...
static float div_expr_safe(Vector2<float> *v, int i, int j, int dim_x,
                           int dim_y, void *ctx)
{
...
static float div_expr_fast(Vector2<float> *v, int i, int j, int dim_x,
                           int dim_y, void *ctx)
{
    float two_dx_inv = *((float*)ctx);
                                         ...
                                         int dim_x, int dim_y, void *ctx)
{
    ...
    float grad_x = (p_right - p_left) * two_dx_inv;
    float grad_y = (p_up - p_down) * two_dx_inv;
    return Vector2<float>(v->x - grad_x, v->y - grad_y);
}
                                         ...
                                         int dim_x, int dim_y, void *ctx)
{
    ...
    float grad_x = (*(p + 1) - *(p - 1)) * two_dx_inv;
    float grad_y = (*(p + dim_x) - *(p - dim_x)) * two_dx_inv;
    return Vector2<float>(v->x - grad_x, v->y - grad_y);
}
...
void subtract_gradient(Vector2<float> *v, float *p, int dim_x, int dim_y,
                       float dx)
{
    struct grad_sub_context pois_ctx = { .p = p,
